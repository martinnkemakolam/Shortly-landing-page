...
import iconPintrest from './images/icon-pinterest.svg'
...
function App() {
  ...
  let jsxMap = arr.map((item, id)=>{
    return(
      <div className='shorted' key={id}>
      <span className='preLink'>
      {item.former}
      </span>
      <div className='newLink'>
        ...
        <button onClick={()=> clipBoardWrite(item.newLink)}>
          Copy
        </button>
      </div>
  </div>
  ...
  async function fetcher(url){
    if(input.current.value>0){
      ...
      .then((res)=>{
        setArr(arr.concat({
          newLink: res.result.short_link,
          former: res.result.original_link
        }))
      }) 
    }else{
  ...
  return (
    <div className="App">
      <header>
        ...
        <div className='linksAndLogin' style={{'top':  active?'100px' :'-500px'}}>
          <div className='links'>
            <span className='logo'>Features</span>
            <span className='logo'>Price</span>
            <span className='logo'>Resources</span>
          </div>
          <div className='login'>
            <span>Login</span>
            <span>Sign Up</span>
        ...
        <div className='click' onClick={()=>setActive(!active)}>
          <div></div>
          <div></div>
          <div></div>
        </div>
      ...
      <section className='getStarted'>
        ...
        <img src={workingIllustration} alt='get started'></img>
      </section>
      <div className='sticky'>
        <form onSubmit={submit}>
          <input className={className ? 'add': ''} ref={input} onChange={()=> input.current.value.length > 0? setClassName(false): setClassName(true)} placeholder='Shorten a link here...'></input>
          <span className='hide' style={{'display': className?'block':'none'}}>Put a valid URL</span>
      ...
      <section className='advancedStats'>
        ...
        <div className='fancyComp'>
          <div className='comp'>
            <img src={iconRecognition} alt='fancyLogo'></img>
          ...
          <div className='comp two'>
            <img src={iconRecord} alt='fancyLogo'></img>
            <h3>Brand Recognition</h3>
            <p>Boost your brand recognition with each click. Generic links don't mean a thing. Branded links help instill confidence in your content.</p>
          </div>
          <div className='comp three'>
            <img src={iconCustomize} alt='fancyLogo'></img>
            <h3>Brand Recognition</h3>
            <p>Boost your brand recognition with each click. Generic links don't mean a thing. Branded links help instill confidence in your content.</p>
          </div>
        </div>
      ...
      <section className='tinyGetStarted'>
      ...
      <footer>
        ...
        <div className='footerLogo'>
          ...
          <img src={iconPintrest} alt='Pintrest icon'></img>
